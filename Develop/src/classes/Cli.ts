...
class Cli {
  // Updated the vehicles property to accept Truck and Motorbike objects as well by using the Union operator to define additional types for the array
    ...
    inquirer
      ...
      .then((answers) => {
        if (answers.vehicleType === 'Car') {
          // create a car
          this.createCar();
        } // Statements to create a truck or motorbike
        ...
        // Statements to perform the tow action only if the selected vehicle is a truck.
          ...
          for (let i = 0; i < this.vehicles.length; i++) {
            if (this.vehicles[i].vin === this.selectedVehicleVin && this.vehicles[i] instanceof Truck) {
              this.findVehicleToTow(this.vehicles[i] as Truck); // Pass the truck to findVehicleToTow
              return; 
            }
          }
